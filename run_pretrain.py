...
for ii in range(args.itr):
    ...
    if args.model == 'Autoformer':
        model = Autoformer.Model(args).float() # 创建模型实例
    ...
    args.content = load_content(args) # domain information
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
            ...
            # decoder input =已知的标签部分和需要预测的部分（用零填充）
            ...
            if args.use_amp:
                with torch.cuda.amp.autocast():
                    if args.output_attention: # 是否需要输出注意力权重
...
accelerator.wait_for_everyone() # 分布式训练时等待别的进程
