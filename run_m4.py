...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        exp_folder = os.path.join(base_folder, setting)
        if not os.path.exists(exp_folder):
            os.makedirs(exp_folder)
        
        # 创建或更新 average.xlsx 文件
        average_file = os.path.join(exp_folder, "average.xlsx")
        data = {
            "Epoch": torch.tensor([epoch + 1]),
            "Train Loss": torch.tensor([train_loss]),
            "SMAPE": torch.tensor([test_loss])
        }
